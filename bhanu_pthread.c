...
void report_progress(int thread_id, int total_chunks);
...
void *process_chunk(void *arg) {
    ...
    // Parallel processing of each record in this chunk
    for (int i = chunk_id; i < CHUNK_SIZE; i += num_threads) {
    ...
    // Report progress after processing this chunk
    report_progress(chunk_id, total_chunks);
...
void report_progress(int thread_id, int total_chunks) {
    printf("Thread %d - Processed up to chunk %d\n", thread_id, total_chunks);
}
